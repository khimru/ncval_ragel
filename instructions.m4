...
# already processed by ｢instructions_define｣ and then ｢_instruction_name_action｣
# if expanded iff instruction with the same name was not already processed.
...
｣, ｢)｢｣instruction(｣)｣｢)｣)
################################################################################
# ｢instruction｣ expands to defenition of a single instruction.  This may be few
# lines in the output if the instruction have different variants (32bit/64bit or
# two-byte VEX and three-byte VEX, etc).
# For example: ｢instruction(adc Ib Ev, 0x83 /2, lock)｣
#     becomes: ｢((( data16 )) rex_xb? (0x83) >begin_opcode
...
#		( rex_xb? (0x83) >begin_opcode
#		  (( opcode_2 @end_opcode) @instruction_adc @operands_count_is_2
#		  @operand0_32bit @operand1_8bit @operand1_immediate any* &
#		  ( modrm_registers @operand0_from_modrm_rm | (modrm_memory &
#		  (any @operand0_rm any*)) )) imm8) |
#		((( lock )) rex_xb? (0x83) >begin_opcode
#		  (( opcode_2 @end_opcode) @instruction_adc @operands_count_is_2
#		  @operand0_32bit @operand1_8bit @operand1_immediate any* &
#		  ( (modrm_memory & (any @operand0_rm any*)) )) imm8) |
#		( REXW_XB (0x83) >begin_opcode
#		  (( opcode_2 @end_opcode) @instruction_adc @operands_count_is_2
#		  @operand0_64bit @operand1_8bit @operand1_immediate any* &
#		  ( modrm_registers @operand0_from_modrm_rm | (modrm_memory &
#		  (any @operand0_rm any*)) )) imm8) |
#		((( lock )) REXW_XB (0x83) >begin_opcode
#		  (( opcode_2 @end_opcode) @instruction_adc @operands_count_is_2
#		  @operand0_64bit @operand1_8bit @operand1_immediate any* &
#		  ( (modrm_memory & (any @operand0_rm any*)) )) imm8)｣
...
################################################################################
  define(｢instruction｣, ｢ifelse(｢$1｣, , ,
    ｢instruction_select(possible_command_modes(shift(split_argument($1))),
		    possible_rex_rxb_bits(shift(split_argument($1))), $@)ifelse(
     two_byte_vex(split_argument(｢$2｣)), , , ｢instruction(｢$1｣,
		     two_byte_vex(split_argument(｢$2｣)), shift(shift($@)))｣)｣)｣)
################################################################################
# ｢two_byte_vex｣ expands to alternate (shortened) form of the VEX instruction 
# opcode if it exist.  For non-VEX instructions and for instructions which don't
# have a short form it expands to nothing.
# For example: ｢two_byte_vex(｢0xc4｣, ｢RXB.00001｣, ｢X.src.L.01｣, ｢0x58｣)｣
#     becomes: ｢0xc5 R.src.L.01 0x58 0x00｣.
################################################################################
  define(｢two_byte_vex｣, ｢ifelse(
    ｢$1 $2 ｣substr(｢$3｣, 0, 2), ｢0xc4 RXB.01 0.｣, ｢_two_byte_vex($@)｣,
    ｢$1 $2 ｣substr(｢$3｣, 0, 2), ｢0xc4 RXB.01 x.｣, ｢_two_byte_vex($@)｣,
    ｢$1 $2 ｣substr(｢$3｣, 0, 2), ｢0xc4 RXB.01 X.｣, ｢_two_byte_vex($@)｣,
    ｢$1 $2 ｣substr(｢$3｣, 0, 2), ｢0xc4 RXB.00001 0.｣, ｢_two_byte_vex($@)｣,
    ｢$1 $2 ｣substr(｢$3｣, 0, 2), ｢0xc4 RXB.00001 x.｣, ｢_two_byte_vex($@)｣, )｣)
    ｢$1 $2 ｣substr(｢$3｣, 0, 2), ｢0xc4 RXB.00001 X.｣, ｢_two_byte_vex($@)｣, )｣)
  define(｢_two_byte_vex｣, ｢｢0xc5 R.｣substr(｢$3｣,
		     2) unbrace(translit((shift(shift(shift($@)))), ｢,｣, ｢ ｣))｣)
################################################################################
...
################################################################################
  define(｢return_operand_mode｣, ｢ifelse(
    ...
    ｢ifelse(
      ...
      $1, ｢rexw｣, ｢rexw｣,
      $1, ｢lsetunset｣, ｢lsetunset｣,
      $1, ｢lsetunsetnodataprefixrexw｣, ｢lsetunsetnodataprefixrexw｣,
...
# operands and returns combined list or aborts M4 if they are not compatible.
################################################################################
  define(｢check_prefixes_compatibility｣, ｢ifelse(
    ...
    $1 $2, ｢nodataprefixrexw lsetunset｣, ｢lsetunsetnodataprefixrexw｣,
    $2 $1, ｢nodataprefixrexw lsetunset｣, ｢lsetunsetnodataprefixrexw｣,
...
################################################################################
  ...
  define(｢possible_command_mode_2｣, ｢unknown｣)
  define(｢possible_command_mode_7｣, ｢unknown｣)
  define(｢possible_command_mode_b｣, ｢unknown｣)
  define(｢possible_command_mode_d｣, ｢unknown｣)
  define(｢possible_command_mode_do｣, ｢unknown｣)
  define(｢possible_command_mode_dq｣, ｢unknown｣)
  define(｢possible_command_mode_fq｣, ｢unknown｣)
  define(｢possible_command_mode_o｣, ｢unknown｣)
  define(｢possible_command_mode_p｣, ｢unknown｣)
  define(｢possible_command_mode_pb｣, ｢unknown｣)
  define(｢possible_command_mode_pd｣, ｢unknown｣)
  define(｢possible_command_mode_pdw｣, ｢unknown｣)
  define(｢possible_command_mode_pdwx｣, ｢lsetunset｣)
  define(｢possible_command_mode_pdx｣, ｢lsetunset｣)
  define(｢possible_command_mode_ph｣, ｢unknown｣)
  define(｢possible_command_mode_phx｣, ｢unknown｣)
  define(｢possible_command_mode_pi｣, ｢unknown｣)
  define(｢possible_command_mode_pj｣, ｢unknown｣)
  define(｢possible_command_mode_pjx｣, ｢lsetunset｣)
  define(｢possible_command_mode_pk｣, ｢unknown｣)
  define(｢possible_command_mode_pkx｣, ｢lsetunset｣)
  define(｢possible_command_mode_pq｣, ｢unknown｣)
  define(｢possible_command_mode_pqw｣, ｢unknown｣)
  define(｢possible_command_mode_pqwx｣, ｢lsetunset｣)
  define(｢possible_command_mode_pqx｣, ｢lsetunset｣)
  define(｢possible_command_mode_ps｣, ｢unknown｣)
  define(｢possible_command_mode_psx｣, ｢lsetunset｣)
  define(｢possible_command_mode_pw｣, ｢unknown｣)
  define(｢possible_command_mode_q｣, ｢unknown｣)
  define(｢possible_command_mode_r｣, ｢unknown｣)
  define(｢possible_command_mode_s｣, ｢unknown｣)
  define(｢possible_command_mode_sb｣, ｢unknown｣)
  define(｢possible_command_mode_sd｣, ｢unknown｣)
  define(｢possible_command_mode_se｣, ｢unknown｣)
  define(｢possible_command_mode_si｣, ｢unknown｣)
  define(｢possible_command_mode_sq｣, ｢unknown｣)
  define(｢possible_command_mode_sr｣, ｢unknown｣)
  define(｢possible_command_mode_ss｣, ｢unknown｣)
  define(｢possible_command_mode_st｣, ｢unknown｣)
  define(｢possible_command_mode_sw｣, ｢unknown｣)
  define(｢possible_command_mode_sx｣, ｢unknown｣)
  define(｢possible_command_mode_v｣, ｢data16nodataprefixrexw｣)
  define(｢possible_command_mode_w｣, ｢unknown｣)
  define(｢possible_command_mode_x｣, ｢lsetunset｣)
...
################################################################################
  ...
  define(｢possible_rex_rxb_bits_t｣, )
  define(｢possible_rex_rxb_bits_r｣, ｢ifelse(｢$1｣, 7, , ｢b｣)｣)
  define(｢possible_rex_rxb_bits_B｣, )
  define(｢possible_rex_rxb_bits_C｣, ｢r｣)
  define(｢possible_rex_rxb_bits_D｣, )
  define(｢possible_rex_rxb_bits_E｣, ｢xb｣)
  define(｢possible_rex_rxb_bits_G｣, ｢r｣)
  define(｢possible_rex_rxb_bits_H｣, )
  define(｢possible_rex_rxb_bits_I｣, )
  ...
  define(｢possible_rex_rxb_bits_W｣, ｢xb｣)
...
# It receives five operands: three from command description and two obtained
# from the list of operands.
# For example: ｢instructions(adc Ib Ev, 0x83 /2, lock)｣
#     becomes: ｢instruction_select(｢data16nodataprefixrexw｣, ｢xb｣, ｢adc Ib Ev｣,
#							     ｢0x83 /2｣, ｢lock｣)｣
#    and then: ｢((( data16 )) rex_xb? (0x83) >begin_opcode
#		  (( opcode_2 @end_opcode) @instruction_adc @operands_count_is_2
#		  @operand0_16bit @operand1_8bit @operand1_immediate any* &
#		  ( modrm_registers @operand0_from_modrm_rm | (modrm_memory &
#		  (any @operand0_rm any*)) )) imm8) |
...
################################################################################
  define(｢instruction_select｣, ｢ifelse(
	...
	｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	  ｢$5｣) REX_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢size8｣,
							     shift(shift($@)))｣,
	｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	  ｢$5｣) REX_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢size8｣, shift(shift(
	  $@)))instruction_separator｢｣｢(｣minimum_one_required_prefix(｢lock｣,
	  ｢$5｣) REX_prefix_q(｢$2｣, ｢$4｣)instruction_body(
	  ｢memonlysize8｣, shift(shift($@)))｣)｣,
    ｢$1｣, ｢data16｣,
      ｢ifelse(index(｢$5｣, ｢lock｣), -1,
	｢instruction_separator｢｣｢(｣minimum_one_required_prefix(｢data16｣,
	  ｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢data16｣,
							     shift(shift($@)))｣,
	｢instruction_separator｢｣｢(｣minimum_one_required_prefix(｢data16｣,
	  ｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢data16｣, shift(shift(
	  $@)))instruction_separator｢｣｢(｣minimum_two_required_prefixes(｢data16｣,
	  ｢lock｣, ｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(
					  ｢memonlydata16｣, shift(shift($@)))｣)｣,
    ｢$1｣, ｢rexw｣, 
      ｢ifelse(index(｢$5｣, ｢lock｣), -1,
	｢ifelse(substr(｢$4｣, 0, 5), ｢0xc5 ｣, ,
	  ｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	  ｢$5｣) REXW_prefix(｢$2｣, ｢$4｣)instruction_body(｢rexw｣,
							   shift(shift($@)))｣)｣,
	｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	  ｢$5｣) REXW_prefix(｢$2｣, ｢$4｣)instruction_body(｢rexw｣, shift(shift(
	  $@)))instruction_separator｢｣｢(｣minimum_one_required_prefix(｢lock｣,
	  ｢$5｣) REXW_prefix(｢$2｣, ｢$4｣)instruction_body(｢memonlyrexw｣,
	  shift(shift($@)))｣)｣,
    ｢$1｣, ｢nodataprefix｣,
      ｢ifelse(index(｢$5｣, ｢lock｣), -1,
	｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	  ｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢nodataprefix｣,
							     shift(shift($@)))｣,
	｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	  ｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢nodataprefix｣, shift(
	  shift($@)))instruction_separator｢｣｢(｣minimum_one_required_prefix(
	  ｢lock｣, ｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(
				    ｢memonlynodataprefix｣, shift(shift($@)))｣)｣,
    ｢$1｣, ｢lsetunset｣,
      ｢ifelse(index(｢$4｣, ｢.L.｣), -1,
	｢fatal_error(｢Can not find L bit in opcode: $4｣)｣,
	｢instruction_select(｢nodataprefix｣, ｢$2｣, make_operands_128bit(｢$3｣),
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.0.\2｣), shift(shift(shift(shift(
	  $@)))))｣｢instruction_select(｢nodataprefix｣, ｢$2｣, ｢$3｣,
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.1.\2｣), shift(shift(shift(shift(
	  $@)))))｣)｣,
    ｢$1｣, ｢memonlylsetunset｣,
      ｢ifelse(index(｢$4｣, ｢.L.｣), -1,
	｢fatal_error(｢Can not find L bit in opcode: $4｣)｣,
	｢instruction_select(｢memonlynodataprefix｣, ｢$2｣, make_operands_128bit(｢$3｣),
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.0.\2｣), shift(shift(shift(shift(
	  $@)))))｣｢instruction_select(｢memonlynodataprefix｣, ｢$2｣, ｢$3｣,
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.1.\2｣), shift(shift(shift(shift(
	  $@)))))｣)｣,
    ｢$1｣, ｢regonlylsetunset｣,
      ｢ifelse(index(｢$4｣, ｢.L.｣), -1,
	｢fatal_error(｢Can not find L bit in opcode: $4｣)｣,
	｢instruction_select(｢regonlynodataprefix｣, ｢$2｣, make_operands_128bit(｢$3｣),
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.0.\2｣), shift(shift(shift(shift(
	  $@)))))｣｢instruction_select(｢regonlynodataprefix｣, ｢$2｣, ｢$3｣,
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.1.\2｣), shift(shift(shift(shift(
	  $@)))))｣)｣,
    ｢$1｣, ｢lsetunset｣,
      ｢ifelse(index(｢$4｣, ｢.L.｣), -1,
	...
	｢instruction_select(｢nodataprefix｣, ｢$2｣, make_operands_128bit(｢$3｣),
	  ...
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.1.\2｣), shift(shift(shift(shift(
	  $@)))))｣)｣,
    ｢$1｣, ｢lsetunsetnodataprefixrexw｣,
      ｢ifelse(index(｢$4｣, ｢.L.｣), -1,
	｢fatal_error(｢Can not find L bit in opcode: $4｣)｣,
	｢instruction_select(｢nodataprefixrexw｣, ｢$2｣, make_operands_128bit(｢$3｣),
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.0.\2｣), shift(shift(shift(shift(
	  $@)))))｣｢instruction_select(｢nodataprefixrexw｣, ｢$2｣, ｢$3｣,
	  regexp(｢$4｣, ｢\(.*\).L\.\(.*\)｣, ｢\1.1.\2｣), shift(shift(shift(shift(
	  $@)))))｣)｣,
    ｢$1｣, ｢memonlysize8data16nodataprefixrexw｣,
    ...
    ｢$1｣, ｢memonlysize8｣,
      ｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	｢$5｣) REX_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢memonlysize8｣,
							     shift(shift($@)))｣,
    ｢$1｣, ｢memonlydata16｣,
      ｢instruction_separator｢｣｢(｣minimum_one_required_prefix(｢data16｣,
        ｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(
    ...
    ｢$1｣, ｢memonlynodataprefix｣,
      ｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢memonlynodataprefix｣,
							     shift(shift($@)))｣,
    ｢$1｣, ｢regonlysize8data16nodataprefixrexw｣,
    ...
    ｢$1｣, ｢regonlysize8｣,
      ｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	｢$5｣) REX_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢regonlysize8｣,
							     shift(shift($@)))｣,
    ｢$1｣, ｢regonlydata16｣,
      ｢instruction_separator｢｣｢(｣minimum_one_required_prefix(｢data16｣,
	｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(
    ...
    ｢$1｣, ｢regonlyrexw｣, 
      ｢ifelse(substr(｢$4｣, 0, 5), ｢0xc5 ｣, ,
	｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	｢$5｣) REXW_prefix(｢$2｣, ｢$4｣)instruction_body(｢regonlyrexw｣,
							   shift(shift($@)))｣)｣,
    ｢$1｣, ｢regonlynodataprefix｣,
      ｢instruction_separator｢｣｢(｣possible_optional_prefixes(
	｢$5｣) rex_prefix_q(｢$2｣, ｢$4｣)instruction_body(｢regonlynodataprefix｣,
							     shift(shift($@)))｣,
    ｢fatal_error(｢Incorrect operand mode:｣ $1)｣)｣)
################################################################################
# ｢possible_optional_prefixes｣ creates list of possible permutations of optional
# prefixes.
...
################################################################################
  define(｢possible_optional_prefixes｣, ｢ifelse(
    ...
    ｢possible_prefixes(optional_prefixes(｢$1｣))?｣)｣)
################################################################################
# ｢minimum_one_required_prefix｣ is similar to ｢possible_optional_prefixes｣ but
# it accepts two argumetns: first argument is prefix which must always be
# present and second argument is like in ｢possible_optional_prefixes｣
################################################################################
...
################################################################################
# ｢optional_prefixes｣ parses third column of instruction description (see
# details in general-purpose-instructions.def) and pulls possible optional
# prefixes from it.
...
# For example: ｢REX_prefix_q(｢rb｣, ｢0x80｣)｣
#     becomes: ｢REX_RX?｣
# Another example: 
################################################################################
  define(｢rex_prefix_q｣, ｢ifelse(
    substr(｢$2｣, 0, 5), ｢0xc4 ｣, ｢VEX_prefix(0, ｢$1｣, split_argument(｢$2｣))｣,
    substr(｢$2｣, 0, 5), ｢0xc5 ｣, ｢VEX_prefix(0, ｢$1｣, split_argument(｢$2｣))｣,
    ｢$2｣, ｢0x8f /0｣, ｢｢rex_$1?｣ ｣,
    substr(｢$2｣, 0, 5), ｢0x8f ｣, ｢VEX_prefix(0, ｢$1｣, split_argument(｢$2｣))｣,
    $1, , ,
    ｢｢rex_$1?｣ ｣)｣)
  define(｢REX_prefix_q｣, ｢ifelse(
    substr(｢$2｣, 0, 5), ｢0xc4 ｣, ｢VEX_prefix(0, ｢$1｣, split_argument(｢$2｣))｣,
    substr(｢$2｣, 0, 5), ｢0xc5 ｣, ｢VEX_prefix(0, ｢$1｣, split_argument(｢$2｣))｣,
    ｢$2｣, ｢0x8f /0｣, ｢｢REX_｣translit($1, ｢a-z｣, ｢A-Z｣)? ｣,
    substr(｢$2｣, 0, 5), ｢0x8f ｣, ｢VEX_prefix(0, ｢$1｣, split_argument(｢$2｣))｣,
    $1, , ,
    ｢｢REX_｣translit($1, ｢a-z｣, ｢A-Z｣)? ｣)｣)
  define(｢REXW_prefix｣, ｢ifelse(
    substr(｢$2｣, 0, 5), ｢0xc4 ｣, ｢VEX_prefix(1, ｢$1｣, split_argument(｢$2｣))｣,
    substr(｢$2｣, 0, 5), ｢0xc5 ｣, ｢VEX_prefix(1, ｢$1｣, split_argument(｢$2｣))｣,
    ｢$2｣, ｢0x8f /0｣, ｢｢REXW_｣translit($1, ｢a-z｣, ｢A-Z｣) ｣,
    substr(｢$2｣, 0, 5), ｢0x8f ｣, ｢VEX_prefix(1, ｢$1｣, split_argument(｢$2｣))｣,
    $1, , ｢REXW_NONE ｣,
    ｢｢REXW_｣translit($1, ｢a-z｣, ｢A-Z｣) ｣)｣)
################################################################################
# ｢VEX_prefix｣ expands to VEX/XOP prefix (two or three bytes).
# For example: ｢VEX_prefix(｢0｣, ｢rxb｣, ｢0xc4｣, ｢RXB.02｣, ｢W.src1.0.00｣, ｢0xf2｣)｣
#     becomes: ｢(0xc4 (VEX_RXB & VEX_map02) (0x00|0x08|0x10|0x18|0x20|0x28|0x30|
#               0x38|0x40|0x48|0x50|0x58|0x60|0x68|0x70|0x78) @vex_pfx2｣
# Another example: ｢VEX_prefix(｢0｣, ｢rxb｣, ｢0xc5｣, ｢R.src.0.01｣, ｢0x58｣)｣
#	  becomes: ｢(0xc5 (0x01|0x09|0x11|0x19|0x21|0x29|0x31|0x39|0x41|0x49|
#		    0x51|0x59|0x61|0x69|0x71|0x79|0x81|0x89|0x91|0x99|0xa1|
#		    0xa9|0xb1|0xb9|0xc1|0xc9|0xd1|0xd9|0xe1|0xe9|0xf1|0xf9)
#		    @vex_pfx_short｣
################################################################################
  define(｢VEX_prefix｣, ｢ifelse(
    ｢$3｣, ｢0xc4｣, ｢｢(｣_VEX_prefix($@) ｣,
    ｢$3｣, ｢0xc5｣, ｢｢(｣__VEX_prefix($@) ｣,
    ｢$3｣, ｢0x8f｣, ｢｢(｣_VEX_prefix($@) ｣,
    ｢fatal_error(｢Incorrect VEX opcode:｣ $3)｣)｣)
  define(｢_VEX_prefix｣, ｢｢$3 (｣ifelse($2, , VEX_NONE, ｢｢VEX_｣translit($2,
    ｢a-z｣, ｢A-Z｣)｣)｢ & VEX_map｣regexp(｢$4｣, ｢.*\.\(.*\)｣,
					    ｢\1｣)｢) ｣___VEX_prefix(｢$1｣, ｢$5｣)｣)
  define(｢__VEX_prefix｣, ｢ifelse(
    ｢$1｣, 1, ｢fatal_error(｢Short form does not support W != 0, opcode: $*｣)｣,
    index(｢$2｣, ｢r｣), -1,
			｢__VEX_prefix(｢$1｣, ｢r｣, ｢$3｣, translit(｢$4｣, ｢R｣, 1))｣,
    ｢$3 ifelse(regexp(｢$4｣, ｢^\(.*\)\.cntl\.\(.*\)$｣, ｢\1.XXXX.\2｣), ,
      ｢ifelse(regexp(｢$4｣, ｢^\(.*\)\.dest\.\(.*\)$｣, ｢\1.XXXX.\2｣), ,
	｢ifelse(regexp(｢$4｣, ｢^\(.*\)\.src\.\(.*\)$｣, ｢\1.XXXX.\2｣), ,
	  ｢ifelse(regexp(｢$4｣, ｢^\(.*\)\.src1\.\(.*\)$｣, ｢\1.XXXX.\2｣), ,
	    ｢______VEX_prefix(｢$4｣)｣,
	    ｢______VEX_prefix(regexp(｢$4｣, ｢^\(.*\)\.src1\.\(.*\)$｣,
							      ｢\1.XXXX.\2｣))｣)｣,
	  ｢______VEX_prefix(regexp(｢$4｣, ｢^\(.*\)\.src\.\(.*\)$｣,
							      ｢\1.XXXX.\2｣))｣)｣,
	｢______VEX_prefix(regexp(｢$4｣, ｢^\(.*\)\.dest\.\(.*\)$｣,
							      ｢\1.XXXX.\2｣))｣)｣,
      ｢______VEX_prefix(regexp(｢$4｣, ｢^\(.*\)\.cntl\.\(.*\)$｣,
							    ｢\1.XXXX.\2｣))｣)｣)｣)
  define(｢___VEX_prefix｣, ｢ifelse(
    regexp(｢$2｣, ｢^\(W\)\(\..*\)$｣, ｢$1\2｣), , ｢____VEX_prefix(｢$2｣)｣,
    ｢____VEX_prefix(regexp(｢$2｣, ｢^\(W\)\(\..*\)｣, ｢$1\2｣))｣)｣)
  define(｢____VEX_prefix｣, ｢ifelse(
    regexp(｢$1｣, ｢^\(.*\)\.cntl\.\(.*\)$｣, ｢\1.XXXX.\2｣), ,
      ｢ifelse(regexp(｢$1｣, ｢^\(.*\)\.dest\.\(.*\)$｣, ｢\1.XXXX.\2｣), ,
	｢ifelse(regexp(｢$1｣, ｢^\(.*\)\.src\.\(.*\)$｣, ｢\1.XXXX.\2｣), ,
	  ｢ifelse(regexp(｢$1｣, ｢^\(.*\)\.src1\.\(.*\)$｣, ｢\1.XXXX.\2｣), ,
	    ｢_____VEX_prefix(｢$1｣)｣,
	    ｢_____VEX_prefix(regexp(｢$1｣, ｢^\(.*\)\.src1\.\(.*\)$｣,
							      ｢\1.XXXX.\2｣))｣)｣,
	  ｢_____VEX_prefix(regexp(｢$1｣, ｢^\(.*\)\.src\.\(.*\)$｣,
							      ｢\1.XXXX.\2｣))｣)｣,
	｢_____VEX_prefix(regexp(｢$1｣, ｢^\(.*\)\.dest\.\(.*\)$｣,
							      ｢\1.XXXX.\2｣))｣)｣,
      ｢_____VEX_prefix(regexp(｢$1｣, ｢^\(.*\)\.cntl\.\(.*\)$｣, ｢\1.XXXX.\2｣))｣)｣)
  define(｢_____VEX_prefix｣, ｢ifelse(
    ｢$1｣, ｢0.1111.0.00｣, ｢0x78 vex_pfx2｣,
    ｢$1｣, ｢0.1111.0.01｣, ｢0x79 vex_pfx2｣,
    ｢$1｣, ｢0.1111.0.10｣, ｢0x7a vex_pfx2｣,
    ｢$1｣, ｢0.1111.0.11｣, ｢0x7b vex_pfx2｣,
    ｢$1｣, ｢0.1111.1.00｣, ｢0x7c vex_pfx2｣,
    ｢$1｣, ｢0.1111.1.01｣, ｢0x7d vex_pfx2｣,
    ｢$1｣, ｢0.1111.1.10｣, ｢0x7e vex_pfx2｣,
    ｢$1｣, ｢0.1111.1.11｣, ｢0x7f vex_pfx2｣,
    ｢$1｣, ｢0.1111.x.00｣, ｢0x78 vex_pfx2｣,
    ｢$1｣, ｢0.1111.x.01｣, ｢0x79 vex_pfx2｣,
    ｢$1｣, ｢0.1111.x.10｣, ｢0x7a vex_pfx2｣,
    ｢$1｣, ｢0.1111.x.11｣, ｢0x7b vex_pfx2｣,
    ｢$1｣, ｢0.1111.X.00｣, ｢chartest(｢(c & 0xfb) == 0x78｣) vex_pfx2｣,
    ｢$1｣, ｢0.1111.X.01｣, ｢chartest(｢(c & 0xfb) == 0x79｣) vex_pfx2｣,
    ｢$1｣, ｢0.1111.X.10｣, ｢chartest(｢(c & 0xfb) == 0x7a｣) vex_pfx2｣,
    ｢$1｣, ｢0.1111.X.11｣, ｢chartest(｢(c & 0xfb) == 0x7b｣) vex_pfx2｣,
    ｢$1｣, ｢1.1111.0.00｣, ｢0xf8 vex_pfx2｣,
    ｢$1｣, ｢1.1111.0.01｣, ｢0xf9 vex_pfx2｣,
    ｢$1｣, ｢1.1111.0.10｣, ｢0xfa vex_pfx2｣,
    ｢$1｣, ｢1.1111.0.11｣, ｢0xfb vex_pfx2｣,
    ｢$1｣, ｢1.1111.1.00｣, ｢0xfc vex_pfx2｣,
    ｢$1｣, ｢1.1111.1.01｣, ｢0xfd vex_pfx2｣,
    ｢$1｣, ｢1.1111.1.10｣, ｢0xfe vex_pfx2｣,
    ｢$1｣, ｢1.1111.1.11｣, ｢0xff vex_pfx2｣,
    ｢$1｣, ｢1.1111.x.00｣, ｢0xf8 vex_pfx2｣,
    ｢$1｣, ｢1.1111.x.01｣, ｢0xf9 vex_pfx2｣,
    ｢$1｣, ｢1.1111.x.10｣, ｢0xfa vex_pfx2｣,
    ｢$1｣, ｢1.1111.x.11｣, ｢0xfb vex_pfx2｣,
    ｢$1｣, ｢1.1111.X.00｣, ｢chartest(｢(c & 0xfb) == 0xf8｣) vex_pfx2｣,
    ｢$1｣, ｢1.1111.X.01｣, ｢chartest(｢(c & 0xfb) == 0xf9｣) vex_pfx2｣,
    ｢$1｣, ｢1.1111.X.10｣, ｢chartest(｢(c & 0xfb) == 0xfa｣) vex_pfx2｣,
    ｢$1｣, ｢1.1111.X.11｣, ｢chartest(｢(c & 0xfb) == 0xfb｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.0.00｣, ｢chartest(｢(c & 0x87) == 0x00｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.0.01｣, ｢chartest(｢(c & 0x87) == 0x01｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.1.00｣, ｢chartest(｢(c & 0x87) == 0x04｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.1.01｣, ｢chartest(｢(c & 0x87) == 0x05｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.x.00｣, ｢chartest(｢(c & 0x87) == 0x00｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.x.01｣, ｢chartest(｢(c & 0x87) == 0x01｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.x.10｣, ｢chartest(｢(c & 0x87) == 0x02｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.x.11｣, ｢chartest(｢(c & 0x87) == 0x03｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.X.00｣, ｢chartest(｢(c & 0x83) == 0x00｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.X.01｣, ｢chartest(｢(c & 0x83) == 0x01｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.X.10｣, ｢chartest(｢(c & 0x83) == 0x02｣) vex_pfx2｣,
    ｢$1｣, ｢0.XXXX.X.11｣, ｢chartest(｢(c & 0x83) == 0x03｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.0.00｣, ｢chartest(｢(c & 0x87) == 0x80｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.0.01｣, ｢chartest(｢(c & 0x87) == 0x81｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.1.00｣, ｢chartest(｢(c & 0x87) == 0x84｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.1.01｣, ｢chartest(｢(c & 0x87) == 0x85｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.x.00｣, ｢chartest(｢(c & 0x87) == 0x80｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.x.01｣, ｢chartest(｢(c & 0x87) == 0x81｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.x.10｣, ｢chartest(｢(c & 0x87) == 0x82｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.x.11｣, ｢chartest(｢(c & 0x87) == 0x83｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.X.00｣, ｢chartest(｢(c & 0x83) == 0x80｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.X.01｣, ｢chartest(｢(c & 0x83) == 0x81｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.X.10｣, ｢chartest(｢(c & 0x83) == 0x82｣) vex_pfx2｣,
    ｢$1｣, ｢1.XXXX.X.11｣, ｢chartest(｢(c & 0x83) == 0x83｣) vex_pfx2｣,
    ｢$1｣, ｢x.1111.0.00｣, ｢0x78 vex_pfx2｣,
    ｢$1｣, ｢x.1111.0.01｣, ｢0x79 vex_pfx2｣,
    ｢$1｣, ｢x.1111.0.10｣, ｢0x7a vex_pfx2｣,
    ｢$1｣, ｢x.1111.0.11｣, ｢0x7b vex_pfx2｣,
    ｢$1｣, ｢x.1111.1.00｣, ｢0x7c vex_pfx2｣,
    ｢$1｣, ｢x.1111.1.01｣, ｢0x7d vex_pfx2｣,
    ｢$1｣, ｢x.1111.1.10｣, ｢0x7e vex_pfx2｣,
    ｢$1｣, ｢x.1111.1.11｣, ｢0x7f vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.0.00｣, ｢chartest(｢(c & 0x87) == 0x00｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.0.01｣, ｢chartest(｢(c & 0x87) == 0x01｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.0.10｣, ｢chartest(｢(c & 0x87) == 0x02｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.0.11｣, ｢chartest(｢(c & 0x87) == 0x03｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.1.00｣, ｢chartest(｢(c & 0x87) == 0x04｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.1.01｣, ｢chartest(｢(c & 0x87) == 0x05｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.1.10｣, ｢chartest(｢(c & 0x87) == 0x06｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.1.11｣, ｢chartest(｢(c & 0x87) == 0x07｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.x.00｣, ｢chartest(｢(c & 0x87) == 0x00｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.x.01｣, ｢chartest(｢(c & 0x87) == 0x01｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.x.10｣, ｢chartest(｢(c & 0x87) == 0x02｣) vex_pfx2｣,
    ｢$1｣, ｢x.XXXX.x.11｣, ｢chartest(｢(c & 0x87) == 0x03｣) vex_pfx2｣,
    ｢$1｣, ｢X.1111.0.00｣, ｢chartest(｢(c & 0x7f) == 0x78｣) vex_pfx2｣,
    ｢$1｣, ｢X.1111.0.01｣, ｢chartest(｢(c & 0x7f) == 0x79｣) vex_pfx2｣,
    ｢$1｣, ｢X.1111.0.10｣, ｢chartest(｢(c & 0x7f) == 0x7a｣) vex_pfx2｣,
    ｢$1｣, ｢X.1111.0.11｣, ｢chartest(｢(c & 0x7f) == 0x7b｣) vex_pfx2｣,
    ｢$1｣, ｢X.1111.1.00｣, ｢chartest(｢(c & 0x7f) == 0x7c｣) vex_pfx2｣,
    ｢$1｣, ｢X.1111.1.01｣, ｢chartest(｢(c & 0x7f) == 0x7d｣) vex_pfx2｣,
    ｢$1｣, ｢X.1111.1.10｣, ｢chartest(｢(c & 0x7f) == 0x7e｣) vex_pfx2｣,
    ｢$1｣, ｢X.1111.1.11｣, ｢chartest(｢(c & 0x7f) == 0x7f｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.0.00｣, ｢chartest(｢(c & 0x07) == 0x00｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.0.01｣, ｢chartest(｢(c & 0x07) == 0x01｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.0.10｣, ｢chartest(｢(c & 0x07) == 0x02｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.0.11｣, ｢chartest(｢(c & 0x07) == 0x03｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.1.00｣, ｢chartest(｢(c & 0x07) == 0x04｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.1.01｣, ｢chartest(｢(c & 0x07) == 0x05｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.1.10｣, ｢chartest(｢(c & 0x07) == 0x06｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.1.11｣, ｢chartest(｢(c & 0x07) == 0x07｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.X.00｣, ｢chartest(｢(c & 0x03) == 0x00｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.X.01｣, ｢chartest(｢(c & 0x03) == 0x01｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.X.10｣, ｢chartest(｢(c & 0x03) == 0x02｣) vex_pfx2｣,
    ｢$1｣, ｢X.XXXX.X.11｣, ｢chartest(｢(c & 0x03) == 0x03｣) vex_pfx2｣,
    ｢fatal_error(｢Unknown second VEX/XOP prefix:｣ $1)｣)｣)
  define(｢______VEX_prefix｣, ｢ifelse(
    ｢$1｣, ｢1.1111.0.00｣, ｢0xf8 vex_pfx_short｣,
    ｢$1｣, ｢1.1111.1.00｣, ｢0xfc vex_pfx_short｣,
    ｢$1｣, ｢1.XXXX.0.00｣, ｢chartest(｢(c & 0x87) == 0x80｣) vex_pfx_short｣,
    ｢$1｣, ｢1.XXXX.0.01｣, ｢chartest(｢(c & 0x87) == 0x81｣) vex_pfx_short｣,
    ｢$1｣, ｢1.XXXX.0.10｣, ｢chartest(｢(c & 0x87) == 0x82｣) vex_pfx_short｣,
    ｢$1｣, ｢1.XXXX.0.11｣, ｢chartest(｢(c & 0x87) == 0x83｣) vex_pfx_short｣,
    ｢$1｣, ｢R.1111.0.00｣, ｢chartest(｢(c & 0x7f) == 0x78｣) vex_pfx_short｣,
    ｢$1｣, ｢R.1111.0.01｣, ｢chartest(｢(c & 0x7f) == 0x79｣) vex_pfx_short｣,
    ｢$1｣, ｢R.1111.0.10｣, ｢chartest(｢(c & 0x7f) == 0x7a｣) vex_pfx_short｣,
    ｢$1｣, ｢R.1111.0.11｣, ｢chartest(｢(c & 0x7f) == 0x7b｣) vex_pfx_short｣,
    ｢$1｣, ｢R.1111.1.00｣, ｢chartest(｢(c & 0x7f) == 0x7c｣) vex_pfx_short｣,
    ｢$1｣, ｢R.1111.1.01｣, ｢chartest(｢(c & 0x7f) == 0x7d｣) vex_pfx_short｣,
    ｢$1｣, ｢R.1111.1.10｣, ｢chartest(｢(c & 0x7f) == 0x7e｣) vex_pfx_short｣,
    ｢$1｣, ｢R.1111.1.11｣, ｢chartest(｢(c & 0x7f) == 0x7f｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.0.00｣, ｢chartest(｢(c & 0x07) == 0x00｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.0.01｣, ｢chartest(｢(c & 0x07) == 0x01｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.0.10｣, ｢chartest(｢(c & 0x07) == 0x02｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.0.11｣, ｢chartest(｢(c & 0x07) == 0x03｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.1.00｣, ｢chartest(｢(c & 0x07) == 0x04｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.1.01｣, ｢chartest(｢(c & 0x07) == 0x05｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.1.10｣, ｢chartest(｢(c & 0x07) == 0x06｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.1.11｣, ｢chartest(｢(c & 0x07) == 0x07｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.x.00｣, ｢chartest(｢(c & 0x07) == 0x00｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.x.01｣, ｢chartest(｢(c & 0x07) == 0x01｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.x.10｣, ｢chartest(｢(c & 0x07) == 0x02｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.x.11｣, ｢chartest(｢(c & 0x07) == 0x03｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.X.00｣, ｢chartest(｢(c & 0x03) == 0x00｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.X.01｣, ｢chartest(｢(c & 0x03) == 0x01｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.X.10｣, ｢chartest(｢(c & 0x03) == 0x02｣) vex_pfx_short｣,
    ｢$1｣, ｢R.XXXX.X.11｣, ｢chartest(｢(c & 0x03) == 0x03｣) vex_pfx_short｣,
    ｢fatal_error(｢Unknown second VEX/XOP prefix:｣ $1)｣)｣)
################################################################################
...
################################################################################
# ｢make_operands_128bit｣ makes all 256-capable vector operands 128bit.
# For example: ｢make_operands_128bit(｢Wpdx Hpdx Vpdx｣)｣
#     becomes: ｢Wpd Hpd Vpd｣.
################################################################################
define(｢make_operands_128bit｣,
  ｢ifelse(regexp(｢$1｣, ｢\(.*[HLUVW]p?.?.?\)x\(.*\)｣, ｢\1\2｣), , ｢$1｣,
  ｢make_operands_128bit(regexp(｢$1｣, ｢\(.*[HLUVW]p?.?.?\)x\(.*\)｣, ｢\1\2｣))｣)｣)
################################################################################
...
# include “/0”, …, “/7” “bytes” then it does most of the ｢instruction_body｣
...
################################################################################
  ...
  define(｢__opcode_nomodrm｣,
    ｢___opcode_nomodrm(｢$3｣) end_opcode(｢$4｣) ifelse(
    ...
    $@)))))｣)
  define(｢___opcode_nomodrm｣, ｢ifelse(
    substr(｢$1｣, 0, 5), ｢0xc4 ｣,
      ｢begin_opcode(unbrace(translit((shift(shift(shift(split_argument(
					       ｢$1｣)))｢｢｢｢)｣｣｣｣)), ｢,｣, ｢ ｣)))｣,
    substr(｢$1｣, 0, 5), ｢0xc5 ｣,
      ｢begin_opcode(unbrace(translit((shift(shift(split_argument(
						｢$1｣))｢｢｢｢)｣｣｣｣)), ｢,｣, ｢ ｣)))｣,
    substr(｢$1｣, 0, 5), ｢0x8f ｣,
        ｢begin_opcode(unbrace(translit((shift(shift(shift(split_argument(
						｢$1｣)))｢｢｢｢)｣｣｣｣)), ｢,｣, ｢ ｣)))｣,
...
################################################################################
# ｢clean_name｣ is simple helper function.  It replaces symbols which can be
# found in the instruction name but not in a C identifier with “_”.  We don't
# use any mangling schemes: collisions will be found by C compiler and are
# pretty easy to fix.
...
# an xchg.
# Note: five spaces at the beginning of the generated sequence are important!
# They are trimmed out before final output but they mark opcode as “untouchable”
# in ___opcode_nomodrm.
################################################################################
...
################################################################################
  define(｢instruction_arguments_sizes｣, ｢ifelse(
      ｢$2｣, , ,
      ...
      substr(｢$1｣, 0, 7), ｢regonly｣,
		      ｢instruction_arguments_sizes(substr(｢$1｣, 7), shift($@))｣,
      ｢ifelse(eval(｢$#>2｣), 1,
	｢instruction_arguments_sizes(｢$1｣, shift(shift($@)))｣) unquote(
  ...
  define(｢instruction_argument_size_nodataprefix_H｣, ｢128bit｣)
  define(｢instruction_argument_size_nodataprefix_Hx｣, ｢256bit｣)
  define(｢instruction_argument_size_nodataprefix_L｣, ｢128bit｣)
  define(｢instruction_argument_size_nodataprefix_Lx｣, ｢256bit｣)
  define(｢instruction_argument_size_nodataprefix_V｣, ｢128bit｣)
  define(｢instruction_argument_size_nodataprefix_Vx｣, ｢256bit｣)
  define(｢instruction_argument_size_nodataprefix_W｣, ｢128bit｣)
  define(｢instruction_argument_size_nodataprefix_Wx｣, ｢256bit｣)
  define(｢instruction_argument_size_2｣, ｢2bit｣)
  define(｢instruction_argument_size_7｣, ｢x87｣)
  ...
  define(｢instruction_argument_size_pb｣, ｢xmm｣)
  define(｢instruction_argument_size_pd｣, ｢xmm｣)
  define(｢instruction_argument_size_pdw｣, ｢xmm｣)
  define(｢instruction_argument_size_pdwx｣, ｢ymm｣)
  define(｢instruction_argument_size_pdx｣, ｢ymm｣)
  define(｢instruction_argument_size_ph｣, ｢xmm｣)
  define(｢instruction_argument_size_phx｣, ｢xmm｣)
  define(｢instruction_argument_size_pi｣, ｢xmm｣)
  define(｢instruction_argument_size_pj｣, ｢xmm｣)
  define(｢instruction_argument_size_pjx｣, ｢ymm｣)
  define(｢instruction_argument_size_pk｣, ｢xmm｣)
  define(｢instruction_argument_size_pkx｣, ｢ymm｣)
  define(｢instruction_argument_size_pq｣, ｢xmm｣)
  define(｢instruction_argument_size_pqw｣, ｢xmm｣)
  define(｢instruction_argument_size_pqwx｣, ｢ymm｣)
  define(｢instruction_argument_size_pqx｣, ｢ymm｣)
  define(｢instruction_argument_size_ps｣, ｢xmm｣)
  define(｢instruction_argument_size_psx｣, ｢ymm｣)
  define(｢instruction_argument_size_pw｣, ｢xmm｣)
  ...
  define(｢instruction_argument_size_sb｣, ｢x87_bcd｣)
  define(｢instruction_argument_size_sd｣, ｢float64bit｣)
  define(｢instruction_argument_size_se｣, ｢x87_env｣)
  define(｢instruction_argument_size_si｣, ｢x87_32bit｣)
  define(｢instruction_argument_size_sq｣, ｢x87_64bit｣)
  define(｢instruction_argument_size_sr｣, ｢x87_state｣)
  define(｢instruction_argument_size_ss｣, ｢float32bit｣)
  define(｢instruction_argument_size_st｣, ｢float80bit｣)
  define(｢instruction_argument_size_sw｣, ｢x87_16bit｣)
  define(｢instruction_argument_size_sx｣, ｢x87_mmx_xmm_state｣)
  ...
  define(｢instruction_argument_size_nodataprefix_Uy｣, ｢xmm｣)
...
################################################################################
# ｢instruction_implied_arguments｣ is inserted where we know all the arguments
# and are ready to insert appropriate ragel action.  It's used to generate
# “implied arguments”, i.e. arguments which are always affected by command but
# which are not encoded in command binary representation.
# It receives “instruction mode” and all the arguments in question.
...
################################################################################
  define(｢instruction_implied_arguments｣, ｢ifelse(eval(｢$#>2｣), 1,
    ｢instruction_implied_arguments(｢$1｣, shift(shift($@)))｣)｢｣ifelse(
      ...
      substr(｢$2｣, 0, 1), ｢t｣, ｢ operand｣decr(decr(｢$#｣))｢_st｣,
      substr(｢$2｣, 0, 1), ｢B｣, ｢ operand｣decr(decr(｢$#｣))｢_from_vex｣,
      substr(｢$2｣, 0, 1), ｢C｣, ,
      substr(｢$2｣, 0, 1), ｢D｣, ,
      substr(｢$2｣, 0, 1), ｢E｣, ,
      substr(｢$2｣, 0, 1), ｢G｣, ,
      substr(｢$2｣, 0, 1), ｢H｣, ｢ operand｣decr(decr(｢$#｣))｢_from_vex｣,
...
################################################################################
  define(｢instruction_modrm_arguments｣,
    ｢ifelse(index(｢$2｣, ｢ C｣), -1,
      ｢ifelse(index(｢$2｣, ｢ D｣), -1,
        ｢ifelse(index(｢$2｣, ｢ E｣), -1,
	  ｢ifelse(index(｢$2｣, ｢ G｣), -1,
...
################################################################################
# ｢register_instruction_modrm_arguments｣ used by ｢instruction_modrm_arguments｣
...
# Note: you can redefine ｢register_instruction_modrm_arguments｣ to
# unconditional ｢modrm_registers｣ value if you don't need to process arguments.
################################################################################
  ...
  define(｢_register_instruction_modrm_arguments｣, ｢ifelse(eval(｢$#>1｣), 1,
    ｢_register_instruction_modrm_arguments(shift($@))｣)｢｣ifelse(
      ...
      substr(｢$1｣, 0, 1), ｢W｣, ｢ operand｣decr(｢$#｣)｢_from_modrm_rm｣)｣)
################################################################################
...
################################################################################
  ...
  define(｢_memory_instruction_modrm_arguments｣, ｢ifelse(eval(｢$#>1｣), 1,
    ｢_memory_instruction_modrm_arguments(shift($@))｣)｢｣ifelse(
      ...
      substr(｢$1｣, 0, 1), ｢V｣, ｢ operand｣decr(｢$#｣)｢_from_modrm_reg｣,
      substr(｢$1｣, 0, 1), ｢W｣, ｢ operand｣decr(｢$#｣)｢_rm｣)｣)
################################################################################
...
################################################################################
  define(｢instruction_immediate_arguments｣, ｢ifelse(
	...
	substr(｢$2｣, 0, 1), ｢J｣, ｢ifelse(len(｢$2｣), 1,
	  ...
	  ｢ifelse(
	    ...
	    ｢instruction_jump_arguments_$1_｣substr($2, 1))｣)｣,
	substr(｢$2｣, 0, 1), ｢L｣,
	  ｢ifelse(imm2_argument, ,
	    ｢popdef(｢imm2_argument｣) operand｣decr(decr(｢$#｣))｢_from_is4｣,
  ...
  define(｢instruction_immediate_arguments_nodataprefix_2｣,
		   ｢ifelse(imm2_argument, , , ｢pushdef(｢imm2_argument｣)｣) imm2｣)
  ...
  define(｢instruction_immediate_arguments_nodataprefix_d｣, ｢ imm32｣)
  define(｢instruction_immediate_arguments_rexw_d｣, ｢ imm32｣)
